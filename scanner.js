...
function handleScannedCode(code, source = "camera") {
  ...
  if (dedicatedMode && (!scannerRequestedType || scannerProcessing)) {
    return;
  }
  ...
  const savedEvent = registerAttendanceEvent(employee, source, dedicatedMode ? scannerRequestedType : null);
  const sourceText = source === "manual" ? t("scan.sourceManual") : t("scan.sourceCamera");
