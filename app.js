...
const DEMO_LOCKED_CONTROL_SELECTOR = [
  "#employeeForm button",
  "#manualScanButton",
  "#attendanceEditForm button",
  "#absenceForm button",
  "#companyDayForm button",
  "#vacationAllowanceForm button",
  "#settingsForm button",
  "#importJsonButton",
  ...
  "button[onclick^='editAttendanceEvent']",
  "button[onclick^='deleteAttendanceEvent']",
  "button[onclick^='deleteCompanyDay']",
  "button[onclick^='deleteAbsence']",
  "button[onclick^='editVacationAllowance']"
].join(",");

const DEMO_LOCKED_FIELD_SELECTOR = [
  "#employeeForm input",
  "#employeeForm select",
  "#manualScanInput",
  "#attendanceEditForm input",
  "#attendanceEditForm select",
  "#absenceForm input",
  "#absenceForm select",
  "#companyDayForm input",
...
function initDemoMode() {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (!target || (!target.closest(DEMO_LOCKED_CONTROL_SELECTOR) && !target.closest(DEMO_LOCKED_FIELD_SELECTOR))) {
      return;
    }
  ...
  document.addEventListener("submit", (event) => {
    if (!event.target.matches("#employeeForm, #attendanceEditForm, #absenceForm, #companyDayForm, #vacationAllowanceForm, #settingsForm")) {
      return;
    }
  ...
  const observer = new MutationObserver(() => applyDemoModeRestrictions());
  observer.observe(document.querySelector(".main-content"), { childList: true, subtree: true });
}

function applyDemoModeRestrictions() {
  document.querySelectorAll(DEMO_LOCKED_CONTROL_SELECTOR).forEach((control) => {
    control.classList.add("demo-locked-control");
    control.setAttribute("aria-disabled", "true");
    control.setAttribute("title", t("demo.controlLocked"));
  });

  document.querySelectorAll(DEMO_LOCKED_FIELD_SELECTOR).forEach((field) => {
    field.classList.add("demo-locked-field");
    field.setAttribute("aria-disabled", "true");
    field.setAttribute("title", t("demo.controlLocked"));
    field.tabIndex = -1;
  });
}

function showDemoModeNotice() {
  let toast = document.getElementById("demoModeToast");

  if (!toast) {
    toast = document.createElement("div");
    toast.id = "demoModeToast";
    toast.className = "demo-mode-toast";
    toast.setAttribute("role", "status");
    document.body.appendChild(toast);
  }

  toast.textContent = t("demo.controlLocked");
  toast.classList.add("visible");
  window.clearTimeout(demoToastTimer);
  demoToastTimer = window.setTimeout(() => toast.classList.remove("visible"), 2600);
}

function getAppReferenceDate() {
  return typeof getDemoReferenceDate === "function" ? getDemoReferenceDate() : new Date();
}
...
function initAttendancePages() {
  ...
  if (attendanceDateInput) {
    setDateControlValue(attendanceDateInput, getDateKey(getAppReferenceDate()));
...
function registerAttendanceEvent(employee, source = "camera", forcedType = null) {
  ...
  /* A demóban az alábbi éles mentési útvonal nem fut le. */
...
function renderAttendanceList() {
  ...
  const selectedDateKey = dateInput && getDateControlValue(dateInput)
    ...
    : getDateKey(getAppReferenceDate());
...
function renderOpenDayWarning(dateKey, searchTerm = "") {
  ...
  warningBox.textContent = `Figyelem: ${openSummaries.length} dolgozónál nincs távozás rögzítve.`;
...
function renderDashboardSummary() {
  ...
  const todayKey = getDateKey(getAppReferenceDate());
  const activeEmployees = employees.filter((employee) => employee.active);
...
function calculateDailySummary(summaryData) {
  ...
  if (currentInTime) {
    ...
    if (summaryData.dateKey === getDateKey(getAppReferenceDate())) {
      rawWorkedMinutes += Math.max(0, Math.round((getAppReferenceDate() - currentInTime) / 60000));
    }
  }
...
function calculateEmployeePaymentValues(reportData) {
  ...
  const mealVoucherDays = dailyRows.filter((row) => {
    return Number(row.netWorkedMinutes) > 0;
  }).length;
...
function editAttendanceEvent(eventId) {
  ...
  const arrivalTimeInput = document.getElementById("attendanceEditArrivalTimeInput");
  const departureTimeInput = document.getElementById("attendanceEditDepartureTimeInput");

  if (
    !editBox || !employeeIdInput || !arrivalIdInput || !departureIdInput ||
    !employeeNameBox || !dateInput || !arrivalTimeInput || !departureTimeInput
  ) {
    return;
  }
  ...
  const dayEvents = attendanceEvents
    ...
    .sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
  const arrivalEvent = dayEvents.find((item) => item.type === "in") || null;
  const departureEvents = dayEvents.filter((item) => item.type === "out");
  const departureEvent = departureEvents.length > 0 ? departureEvents[departureEvents.length - 1] : null;
  const employee = employees.find((item) => item.id === selectedEvent.employeeId);

  employeeIdInput.value = selectedEvent.employeeId;
  arrivalIdInput.value = arrivalEvent ? arrivalEvent.id : "";
  departureIdInput.value = departureEvent ? departureEvent.id : "";
  ...
  setTimeControlValue(arrivalTimeInput, arrivalEvent ? formatTimeFromIso(arrivalEvent.createdAt) : "");
  setTimeControlValue(departureTimeInput, departureEvent ? formatTimeFromIso(departureEvent.createdAt) : "");
...
function saveAttendanceEditForm(event) {
  ...
  if (!employeeIdInput || !arrivalIdInput || !departureIdInput || !dateInput || !arrivalTimeInput || !departureTimeInput) {
    return;
  }

  const employeeId = employeeIdInput.value.trim();
  const arrivalId = arrivalIdInput.value.trim();
  const departureId = departureIdInput.value.trim();
  const dateKey = getDateControlValue(dateInput);
  const arrivalTime = arrivalTimeInput.value;
  const departureTime = departureTimeInput.value;

  if (!employeeId || !dateKey || (!arrivalTime && !departureTime)) {
    alert(t("attendance.eventRequired"));
    return;
  }

  if ((arrivalTime && !isValidTimeValue(arrivalTime)) || (departureTime && !isValidTimeValue(departureTime))) {
    alert(t("attendance.invalidTimeFormat"));
    return;
  }

  if (arrivalTime && departureTime && arrivalTime >= departureTime) {
    alert(t("attendance.invalidTimeRange"));
    return;
  }

  const employee = employees.find((item) => item.id === employeeId);
  const referenceEvent = attendanceEvents.find((item) => item.employeeId === employeeId && item.dateKey === dateKey);
...
function upsertAttendanceCorrection({ eventId, employee, referenceEvent, employeeId, dateKey, time, type }) {
  if (!time) {
    return;
  }

  const existingEvent = eventId
    ? attendanceEvents.find((item) => item.id === eventId)
    : null;
  const now = new Date().toISOString();
  const createdAt = buildIsoFromDateAndTime(dateKey, time, existingEvent ? existingEvent.createdAt : now);

  if (existingEvent) {
    attendanceEvents = attendanceEvents.map((item) => item.id === existingEvent.id
      ? { ...item, dateKey, createdAt, updatedAt: now }
      : item);
    return;
  }

  attendanceEvents.push({
    id: `ATT-${Date.now()}-${type}-${Math.random().toString(16).slice(2)}`,
    employeeId,
...
function resetAttendanceEditForm() {
  ...
  [employeeIdInput, arrivalIdInput, departureIdInput].forEach((input) => {
  ...
  setTimeControlValue(arrivalTimeInput, "");
  setTimeControlValue(departureTimeInput, "");
...
function initProfilePage() {
  ...
  if (monthInput) {
    monthInput.value = getMonthKey(getAppReferenceDate());
...
function initStatisticsPage() {
  ...
  if (statisticsDateInput) {
    setDateControlValue(statisticsDateInput, getDateKey(getAppReferenceDate()));
...
function buildMonthlyAccountingRows(monthKey) {
  ...
  return employees
    ...
    .map((employee) => {
      ...
      const mealVoucherDays = employeeDailyRows.filter((row) => Number(row.netWorkedMinutes) > 0).length;
...
function calculateEmployeeAbsenceSummary(employeeId, startKey, endKey) {
  ...
  getAbsencesForEmployeeInRange(employeeId, startKey, endKey).forEach((absence) => {
    ...
    counts[type] += days.length;
    counts.totalDays += days.length;
  });
...
function initVacationTrackerPage() {
  ...
  if (yearInput) {
    yearInput.value = String(getAppReferenceDate().getFullYear());
...
function getVacationTrackerYear() {
  const referenceDate = getAppReferenceDate();
  const yearInput = document.getElementById("vacationYearInput");
  const year = Number(yearInput ? yearInput.value : referenceDate.getFullYear());
